...
| 4 | void *s21_memset(void *str, int c, size_t n) | Копирует символ c (беззнаковый тип) в первые n символов строки, на которую указывает аргумент str. |
...
| 8 | char *s21_strncpy(char *dest, const char *src, size_t n) | Копирует до n символов из строки, на которую указывает src, в dest. |
...
- int s21_sscanf(const char *str, const char *format, ...) - считывает форматированный ввод из строки.
...
* `make s21_string.a` - Сборка библиотеки s21_string.a
...
- Unit-тесты проверяют результаты работы функций путём ее сравнения с реализацией стандартной библиотеки string.h
...
- Поддерживаются следующие дополнительные модификаторы формата:
  - Спецификаторы: g, G, e, E, x, X, o, p, c, d, f, s, u, %
  - Флаги: #, 0, -, +, (пробел)
  - Ширина: *, (число)
  - Точность: .*, .(число)
  - Длина: h, l, L
